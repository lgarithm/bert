...
class ScalingPolicy(Policy):
    def __init__(self, batch_size, num_training_steps, change_step, alpha,
                 path_workers, config_server):
    ...
    def add_worker(self, num_workers):
        ...
        req = urllib.request.Request("http://{}/addworker".format(
            self._config_server),
                                     data=data,
                                     method="POST")
    ...
    def remove_last_worker(self, num_workers):
        try:
            ...
            return
        data = json.dumps(worker).encode("utf-8")
        req = urllib.request.Request("http://{}/removeworker".format(
            self._config_server),
                                     data=data,
                                     method="POST")
        resp = urllib.request.urlopen(req)
